...
- Реализуйте класс `ru.javawebinar.basejava.storage.ArrayStorage` (вставте код в пустые методы класса): работа с `ru.javawebinar.basejava.model.Resume` на основе массива (методы `clear, get, save, delete, getAll, size`)
   - Не используйте в решении коллекции, реализацию на их основе мы добавим позднее
   - Не меняйте синтаксис методов `ru.javawebinar.basejava.storage.ArrayStorage` и не меняйте класс `ru.javawebinar.basejava.model.Resume`. Нужет только код внутри готовых методов `ru.javawebinar.basejava.storage.ArrayStorage`
